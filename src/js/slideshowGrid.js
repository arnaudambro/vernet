  ...
  window.setTimeout(() => {
    ...
    slidesContainers.forEach(container => {
      ...
      [...container.querySelector('.slideshow-grid__content').children].forEach((pic, i) => {
        ...
        ? pic.dataset.order = 1
        : pic.dataset.order = index + 1;
      /*translation to the first */
        pic.style.transform = `translate(${container.dataset.width * (pic.dataset.order - 2)}px)`
      })
...
function handleSlideshow(e, arrow) {
  ...
  containers.forEach(container => {
    ...
    /* translate */
    [...container.querySelector('.slideshow-grid__content').children].forEach((pic, i) => {
      pic.style.transform = `translate(${container.dataset.width * (right ? (pic.dataset.order - 3) : (pic.dataset.order - 1))}px)`
    })
    /*end of slideshow */
    ...
    window.setTimeout(() => {
      ...
      [...container.querySelector('.slideshow-grid__content').children].forEach((pic, i) => {
        ...
        pic.dataset.order = order;
        pic.style.transform = `translate(${container.dataset.width * (order - 2)}px)`
      })
