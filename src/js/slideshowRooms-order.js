...
const arrows = [...document.querySelectorAll('.slideshow-rooms__card--picture-arrow--container')];
// const counters = [...document.querySelectorAll('.slideshow-grid__controls--slide')];
const slidesContainers = [...document.querySelectorAll('.slideshow-rooms__card--picture-container:not(.no-slideshow)')];
// const slidesContainers = [...document.querySelectorAll('.slideshow-rooms__card--picture-container')];
const timer = 5000;
...
function initSlideshows(e) {
  ...
  if (document.querySelectorAll('.slideshow-rooms__card--picture-arrow--container-right')) {

    [...document.querySelectorAll('.slideshow-rooms__card--picture-arrow--container-right')].forEach((arrow, index) => {
      let timer;
  ...
  window.setTimeout(() => {
    ...
    document.querySelectorAll(`.slideshow-rooms__card--picture-container`).forEach(slideshow => {
      let slides = document.querySelectorAll(`.slideshow-rooms__card--picture-container.${counter.dataset.slideshow}`);
      let numberOfSlides = 0;
      /*let arrowRight = document.querySelector(`.slideshow-rooms__card--picture-arrow--container-right.${counter.dataset.slideshow}`)*/
      slides.forEach(slide => {
        numberOfSlides = Math.max(slide.querySelector(`.slideshow-rooms__card--picture-content`).children.length, numberOfSlides);
      })
    ...
    slidesContainers.forEach(container => {
      ...
      [...container.querySelector('.slideshow-rooms__card--picture-content').children].forEach((pic, i) => {
        ...
        const index = i + 1;
        index === (parseInt([...container.querySelector('.slideshow-rooms__card--picture-content').children].length, 10))
        ? pic.style.order = 1
        : pic.style.order = index + 1;
      })

      /*translation to the first */
      container.querySelector('.slideshow-rooms__card--picture-content').style.transform = `translateX(-${container.dataset.width}px)`;
    })
  }, transitionDuration)
};
...
function handleSlideshow(e, arrow) {
  ...
  const right = clicked.classList.contains('slideshow-rooms__card--picture-arrow--container-right');
  const containers = document.querySelectorAll(`.slideshow-rooms__card--picture-container.${clicked.dataset.slideshow}:not(.no-slideshow)`);
  ...
  containers.forEach(container => {
    container.querySelector('.slideshow-rooms__card--picture-content').style.transform = `translateX(-${right * 2 * container.dataset.width}px)`;
    container.querySelector('.slideshow-rooms__card--picture-content').classList.add('active');
    /* flexbox reordering and bring back the translation where it should be*/
    /*const first = 1;*/
    const last = (parseInt([...container.querySelector('.slideshow-rooms__card--picture-content').children].length, 10));
    const currentSlideOrder = 2;
    window.setTimeout(() => {
      container.querySelector('.slideshow-rooms__card--picture-content').classList.remove('active');
      [...container.querySelector('.slideshow-rooms__card--picture-content').children].forEach((pic, i) => {
        const index = i + 1;
